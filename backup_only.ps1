# backup_only.ps1
# Script to create a timestamped ZIP backup of server database and files without updating code

$rootDir = Split-Path -Parent $MyInvocation.MyCommand.Path
if (-not $rootDir) { $rootDir = Get-Location }

Write-Host "📦 Creating server backup in $rootDir..." -ForegroundColor Green
...
if ($itemsToZip.Count -gt 0) {
    try {
        ...
        Write-Host "⚠️ Error creating ZIP backup: $_" -ForegroundColor Red
    }
}
...
Write-Host "🎉 Backup complete! File saved in backups/$zipFileName" -ForegroundColor Green
