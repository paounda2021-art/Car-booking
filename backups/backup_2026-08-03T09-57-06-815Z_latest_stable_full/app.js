  ...
  try {
    ...
    if (dbResponse.ok) {
      ...
      if (dbBookings && Array.isArray(dbBookings)) {
        ...
        // Merge with local storage bookings to ensure newly created or updated local approval states are preserved
          ...
          try {
            ...
            if (Array.isArray(localBookings)) {
              ...
              localBookings.forEach(localB => {
                if (localB && localB.id && localB.id !== 'system_config') {
                  const dbIndex = dbBookings.findIndex(dbB => dbB.id === localB.id);
                  if (dbIndex === -1) {
                    dbBookings.push(localB);
                    hasMergedNew = true;
                  } else {
                    const dbB = dbBookings[dbIndex];
                    const localLevel = localB.currentApprovalLevel || 1;
                    const dbLevel = dbB.currentApprovalLevel || 1;
                    const localSigsCount = (localB.signatures || []).filter(s => s.status === 'approved').length;
                    const dbSigsCount = (dbB.signatures || []).filter(s => s.status === 'approved').length;
                    
                    const isLocalNewer = (localLevel > dbLevel) ||
                      (localSigsCount > dbSigsCount) ||
                      (localB.status === 'approved' && dbB.status !== 'approved') ||
                      (localB.status === 'rejected' && dbB.status !== 'rejected') ||
                      (localB.status === 'cancelled' && dbB.status !== 'cancelled');

                    if (isLocalNewer) {
                      dbBookings[dbIndex] = localB;
                      hasMergedNew = true;
                    }
                  }
                }
              ...
              if (hasMergedNew) {
                console.log("🛡️ Safely merged updated local approval state into fetched database!");
              }
            }
          } catch(e) {}
        }
...
function renderBookingsLists() {
  ...
  bookings.forEach(b => {
    ...
    // 🎯 เงื่อนไข: ใบจองที่จะมาแสดงที่ L2/L3 ในประวัติ ต้องผ่าน L1 เรียบร้อยแล้ว สำหรับ L4 แสดงเฉพาะรายการที่อนุมัติเสร็จสิ้นแล้วเท่านั้น
  ...
  const isL4Only = isL4User && !isL2User && !isL3User;
  const isL2OrL3OrL4 = currentUser && (isL2User || isL3User || isL4User || checkCanSeeAll(currentUser));

  if (historyFilterBar) {
    historyFilterBar.style.display = isL2OrL3OrL4 ? 'flex' : 'none';
  }

  // 🎯 ซ่อน Dropdown ตัวกรองสถานะสำหรับ L4 (ให้เห็นเฉพาะรายการที่อนุมัติเสร็จสิ้นแล้วเท่านั้น)
  if (statusFilterSelect) {
    statusFilterSelect.style.display = (isL2User || isL3User) ? 'inline-block' : 'none';
  }
...
async function handleApprovalAction(isApproved) {
  ...
  // Await saveBookings to guarantee DB & localStorage persist before closing modal
  await saveBookings();
  document.getElementById('modal-approval').classList.remove('active');
  
  if (isApproved) {
    showToast(`ทำรายการอนุมัติระดับ L${level} สำหรับคำขอ ${booking.id} เรียบร้อยแล้ว`, "success");
  } else {
    showToast(`ปฏิเสธคำขอเลขที่ ${booking.id} เรียบร้อยแล้ว`, "info");
  }
...
function setupEventListeners() {
  ...
  document.getElementById('btn-approve-request').addEventListener('click', async () => await handleApprovalAction(true));
  document.getElementById('btn-reject-request').addEventListener('click', async () => await handleApprovalAction(false));
